...
export default {
  ...
  asyncData({ params, error }) {
    return hospApi.getHospitalList(1, 10, null).then((response) => {
      return {
        //现在我们使用的是服务端渲染技术
        //这个list 以前是在 data 中进行定义，现在不用了，可以直接这样
        //content 是 后端的page 对象中的数据
  ...
  methods: {
    init() {
      dictApi.findByDictCode("Hostype").then((response) => {
        ...
        this.hostypeList = [];
        //向hostypeList添加全部值
        //这个页面的'全部' 可以写死的，我这里是想让它和医院等级放到一起
    ...
    //查询医院列表
    getHospInfoList() {
      //这个 hospApi 千万千万不要写成：this.hospApi
      hospApi
        .getHospitalList(this.page, this.limit, this.searchObj)
        .then((response) => {
    ...
    querySearchAsync(queryString, cb) {
      ...
      hospApi.getHospByName(queryString).then((response) => {
    ...
    show(hoscode) {
      //这个是 nuxt 的 动态路由
      //动态路由如何写页面，去 pages 文件夹下，找一个 hospital 文件夹（路径名）
      //然后找 _hoscode.vue    必须以 _ 开头，名字随便写，一般是以参数名
      window.location.href = "/hospital/" + hoscode;

      //这个是 nuxt的固定路由，他会去 pages 文件夹下，找一个 hospital 文件夹，
      //然后默认去找 index.vue
